...
FILES=(runmilo.py milosum.py prepmilo.py plot_traj.py
       test_runmilo.sh test_milosum.sh test_prepmilo.sh install_milo.sh)

echo "expanse:$DEST <- $SRC"
"${SSH[@]}" "$HOST" "mkdir -p '$DEST/examples'" || exit 1
rsync -e "${SSH[*]}" -q "${FILES[@]/#/$SRC/}" "$HOST:$DEST/" || exit 1
rsync -e "${SSH[*]}" -q "$SRC/examples/DA_example.in" "$SRC/examples/DA_ts_freq.out" "$HOST:$DEST/examples/" || exit 1
...
"${SSH[@]}" "$HOST" "bash -lc 'source ~/.bashrc >/dev/null 2>&1; conda activate base >/dev/null 2>&1; \
  bash ~/$DEST/test_runmilo.sh 2>&1 | tail -1; bash ~/$DEST/test_milosum.sh 2>&1 | tail -1; bash ~/$DEST/test_prepmilo.sh 2>&1 | tail -1'"
